    ...
    this.categoryService.list().then((categories: Category[]) => {
      ...
      this.pointsService.list().then((games: GamePlayed[]) => {
        ...
        games.forEach((game) => {
          ...
          if (!this.categoriesCounted.includes(game.categoryId)) {
            this.categoriesCounted.push(game.categoryId);
            this.categoryCounter++;
          }
        });

        this.avgGameSec = this.totalGameSec / this.gameCounter;
        this.categoriesNotPlayed = Math.max(0, this.allCategories.length - this.categoriesCounted.length);
        this.gamesEndInTime = (gamesFinishedOnTime / this.gameCounter) * 100;
